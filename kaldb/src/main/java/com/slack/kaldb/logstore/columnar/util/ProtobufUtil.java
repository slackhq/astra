...
/**
 * Various utilities for working with protocol buffer encoded messages. This utility will look for
 * protobuf class in the configuration. It can be either per Kafka topic configuration, for example:
 * <code>secor.protobuf.message.class.&lt;topic&gt;=&lt;protobuf class name&gt;</code> or, it can be
 * global configuration for all topics (in case all the topics transfer the same message type):
...
public class ProtobufUtil {
  ...
  private Map<String, Class<? extends Message>> messageClassByTopic =
   ...
   * @throws RuntimeException when configuration option <code>secor.protobuf.message.class</code> is
   *     invalid.
   */
    ...
    for (Entry<String, String> entry : messageClassPerTopic.entrySet()) {
      try {
        ...
        Class<? extends Message> messageClass =
            (Class<? extends Message>) Class.forName(entry.getValue());
        Method messageParseMethod =
            messageClass.getDeclaredMethod("parseFrom", new Class<?>[] {byte[].class});
        ...
        if (allTopics) {
          ...
          LOG.info(
              "Using protobuf message class: {} for Kafka topic: {}",
   ...
   * @return protobuf message class used by this utility instance, or <code>null</code> in case
   *     valid class couldn't be found in the configuration.
   */
   ...
   * @param payload Byte array containing encoded json message
  ...
  public Message decodeProtobufMessage(String topic, byte[] payload) {
    Method parseMethod =
        allTopics ? messageParseMethodForAll : messageParseMethodByTopic.get(topic);
    try {
  ...
  /**
   * Decodes protobuf message If the secor.topic.message.format property is set to "JSON" for
   * "topic" assume "payload" is JSON
   *
   * @param topic Kafka topic name
  ...
  public Message decodeProtobufOrJsonMessage(String topic, byte[] payload) {
    try {
      ...
      LOG.debug(
          "Unable to translate JSON string {} to protobuf message",
  ...
  private boolean shouldDecodeFromJsonMessage(String topic) {
    if (StringUtils.isNotEmpty(messageFormatForAll)
        && StringUtils.equalsIgnoreCase(messageFormatForAll, JSON)) {
      return true;
