package com.slack.kaldb.logstore.columnar.common;

import com.slack.kaldb.logstore.columnar.message.ParsedMessage;
 ...
 * decomposing paths.
 *
 * <p>Log file path has the following form:
 * prefix/topic/partition1/.../partitionN/generation_kafkaPartition_firstMessageOffset where: prefix
 * is top-level directory for log files. It can be a local path or an s3 dir, topic is a kafka
 * topic, partition1, ..., partitionN is the list of partition names extracted from message content.
 * E.g., the partition may describe the message date such as dt=2014-01-01, generation is the
 * consumer version. It allows up to perform rolling upgrades of non-compatible Secor releases,
 * kafkaPartition is the kafka partition of the topic, firstMessageOffset is the offset of the first
 * message in a batch of files committed atomically.
 *
...
public class LogFilePath {
  ...
  public LogFilePath(
    ...
    assert kafkaPartitions.length == offsets.length
        : "Size mismatch partitions: "
            + Arrays.toString(kafkaPartitions)
            + " offsets: "
  ...
  public LogFilePath(
    ...
    this(
        prefix,
        message.getTopic(),
        message.getPartitions(),
        generation,
        new int[] {message.getKafkaPartition()},
        new long[] {lastCommittedOffset},
        extension);
  }
  ...
  public LogFilePath withPrefix(String prefix) {
    return new LogFilePath(
        prefix, mTopic, mPartitions, mGeneration, mKafkaPartitions, mOffsets, mExtension);
  }
  ...
  private String getLogFileBasename() {
    ...
    if (mKafkaPartitions.length > 1) {
      String kafkaPartitions =
          mKafkaPartitions[0] + "-" + mKafkaPartitions[mKafkaPartitions.length - 1];
