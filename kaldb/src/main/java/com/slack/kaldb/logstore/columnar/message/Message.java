...
public class Message {
   ...
   * when logging them by using a CharsetDecoder which replaces bad data. (While in practice `new
   * String(bytes)` does the same thing, the documentation for that method leaves that behavior
   * unspecified.) Additionally, in contexts where Message.toString() will be logged at a high level
   * (including exception messages), we truncate long keys and payloads, which may be very long
   * binary data.
   */
  ...
  protected String fieldsToString(boolean truncate) {
    return "topic='"
        ...
        + mOffset
        + ", kafkaKey="
        + bytesToString(mKafkaKey, truncate)
        + ", payload="
        + bytesToString(mPayload, truncate)
        + ", timestamp="
        + mTimestamp
        + ", headers="
        + mHeaders;
