...
public class KaldbDistributedQueryService extends KaldbQueryServiceBase {
  ...
  public KaldbDistributedQueryService(
    ...
    this.searchMetadataStore.addListener(searchServersListener());
    initStubs();
  }

  private void initStubs() {
    List<SearchMetadata> searchMetadataNodes;
    try {
      searchMetadataTotalChangeCounter.increment();
      searchMetadataNodes = searchMetadataStore.list().get();
      searchMetadataNodes.forEach(
          searchMetadata ->
              stubs.put(searchMetadata.url, getKaldbServiceGrpcClient(searchMetadata.url)));
    } catch (Exception e) {
      throw new RuntimeException("Cannot initStubs stubs " + e);
    }
  }

  private KaldbMetadataStoreChangeListener searchServersListener() {
    return () -> {
      try {
        searchMetadataTotalChangeCounter.increment();
        ...
        LOG.info(
            "SearchMetadata listener event. old_count={} new_count={}",
  ...
  private CompletableFuture<List<KaldbSearch.SearchResult>> distributedSearch(
    ...
    try {
      ...
      for (KaldbServiceGrpc.KaldbServiceFutureStub stub : stubs.values()) {
        ...
        // take
        // Alternately use completeOnTimeout and the value can then we configured per request and
        // not
