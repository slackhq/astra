...
public class SearchMetadataStoreManagerTest {
  ...
  public void testCreateAndGetMetadata() throws Exception {
    storeManager = new SearchMetadataStoreManager(curatorFramework, zkConfig, meterRegistry, true);
  ...
  public void testUpdateSearchability() throws Exception {
    storeManager = new SearchMetadataStoreManager(curatorFramework, zkConfig, meterRegistry, true);
    SearchMetadata searchMetadata = new SearchMetadata("test2", "snapshot2", "http://example.com", true);
    storeManager.createSync(searchMetadata);
  ...
  public void testListMetadata() throws Exception {
    ...
    assertThat(metadataList.stream().map(m -> m.name))
        .containsExactlyInAnyOrder("test3", "test4");
  }
  ...
  public void testChangeListener() throws Exception {
    ...
    // Add a listener
    storeManager.addListener((type, metadata) -> {
