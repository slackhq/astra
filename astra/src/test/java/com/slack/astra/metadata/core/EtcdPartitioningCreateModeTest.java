...
public class EtcdPartitioningCreateModeTest {
  ...
  public static void setUpClass() {
    ...
    etcdCluster =
        Etcd.builder().withClusterName("etcd-partition-create-mode-test").withNodes(1).build();
    etcdCluster.start();
    LOG.info(
  ...
  public void testPersistentNode() throws ExecutionException, InterruptedException {
    ...
    try (EtcdPartitioningMetadataStore<TestPartitionedMetadata> persistentStore =
        new EtcdPartitioningMetadataStore<>(
            "/test-persistent", etcdConfig, true, meterRegistry, serializer)) {
  ...
  public void testEphemeralNodeWithShortTtl() throws Exception {
    ...
    try (EtcdPartitioningMetadataStore<TestPartitionedMetadata> newStore =
        new EtcdPartitioningMetadataStore<>(
            "/test-ephemeral", etcdConfig, true, meterRegistry, serializer)) {

      try {
        newStore.getSync(partition, nodeName);
  ...
  public void testMultiplePartitionsWithEphemeralNodes() throws Exception {
    ...
    try (EtcdPartitioningMetadataStore<TestPartitionedMetadata> newStore =
        new EtcdPartitioningMetadataStore<>(
            "/test-multi-partition", etcdConfig, true, meterRegistry, serializer)) {

      // Check all partitions - nodes should be gone
      try {
        newStore.getSync("partition1", "ephemeral1");
        assertThat(false).isTrue(); // This should not be reached
      } catch (RuntimeException e) {
        assertThat(e.getMessage()).contains("Node not found");
      }

      try {
        newStore.getSync("partition2", "ephemeral2");
        assertThat(false).isTrue(); // This should not be reached
      } catch (RuntimeException e) {
        assertThat(e.getMessage()).contains("Node not found");
      }
      ...
      try {
        newStore.getSync("partition3", "ephemeral3");
        assertThat(false).isTrue(); // This should not be reached
      } catch (RuntimeException e) {
        assertThat(e.getMessage()).contains("Node not found");
      }
  ...
  public void testMixedPersistentAndEphemeralNodes() throws Exception {
    ...
    try (EtcdPartitioningMetadataStore<TestPartitionedMetadata> persistentStore =
        new EtcdPartitioningMetadataStore<>(
            "/test-mixed", etcdConfig, true, meterRegistry, serializer)) {
    ...
    try (EtcdPartitioningMetadataStore<TestPartitionedMetadata> newStore =
      ...
      // Ephemeral node should be gone
      try {
        newStore.getSync(partition, "ephemeral1");
        assertThat(false).isTrue(); // This should not be reached
      } catch (RuntimeException e) {
        assertThat(e.getMessage()).contains("Node not found");
      }
